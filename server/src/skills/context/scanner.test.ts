...
describe("SkillScanner public discovery boundary", () => {
  ...
  it("does not expose unregistered system helpers to catalog or Agent matching", async () => {
    ...
    expect(manifests.map((skill) => skill.id)).toEqual(["public-skill"]);
    expect(manifests[0]?.category).toBe("实验");
  });
