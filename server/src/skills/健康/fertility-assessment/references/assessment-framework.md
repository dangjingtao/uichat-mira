...
- 最终报告中的已选择维度必须有 `0~10` 数值分；
- 无直接证据时使用 `5.0 / low` 中性参考基准，并明确显示“暂不可充分评价”；
- `confidence` 与数据完整度必须独立表达，不用一个精确分数掩盖证据不足；
- evidence、concerns、missingEvidence 必须承担不同作用并去重；
- AMH / AFC 主要用于理解卵巢储备与促排反应背景，不得单独等同卵子质量或自然受孕概率；
- 不根据单个指标直接给出个体自然受孕概率。

完整公开标准见 `scoring-standard-v2.md`。
...
- 具体促排、移植、用药、补充剂、手术和检查方向统一转为就诊讨论问题；
- 不承诺某项干预能够提高个人的成功率、妊娠率或活产率；
