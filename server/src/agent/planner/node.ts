...
import type { AgentNextAction, AgentToolExecutionResult } from "../types";
...
type PlannerExecutionObservation = ReturnType<
  typeof buildExecutionObservationView
>[number];
...
const getPlannerObservationTarget = (observation: PlannerExecutionObservation) => {
  if (
...
/**
 * Builds a compact full-run ledger from accumulated Evidence.
 *
 * Recent executionHistory intentionally remains bounded for rich local context, while
 * this ledger survives that rolling window. Repeated actions collapse by semantic target
 * instead of making the Planner re-read an ever-growing raw transcript.
 */
export const buildPlannerAccumulatedActionLedger = (
  observations: PlannerExecutionObservation[],
  charLimit = PLANNER_ACTION_LEDGER_CHAR_LIMIT,
) => {
  const byAction = new Map<
    string,
    {
      actionType: PlannerExecutionObservation["actionType"];
      toolId?: string;
      target?: string;
      status: PlannerExecutionObservation["status"];
      attempts: number;
      inputHashes: string[];
  ...
  for (const observation of observations) {
    const target = getPlannerObservationTarget(observation);
    const semanticKey = [
      observation.actionType,
      observation.toolId ?? "",
      target ?? observation.inputHash ?? observation.id,
    ].join(":");
    const previous = byAction.get(semanticKey);
    const inputHashes = [
      ...(previous?.inputHashes ?? []),
      ...(observation.inputHash ? [observation.inputHash] : []),
    ...
    byAction.set(semanticKey, {
      ...
      inputHashes: inputHashes.map((hash) => hash.slice(0, 16)),
      ...(summary?.actionTaken
        ? {
...
const getExecutionLlmContent = (execution: AgentToolExecutionResult) =>
...
/**
 * Gives Planner the latest real bounded executor payload instead of forcing it to infer
 * the result from a tiny Evidence preview. Historical raw payloads stay out of the prompt;
 * only the latest payload is rich, while the full run is represented by the compact ledger.
 */
export const buildPlannerLatestEvidenceContent = (
  state: AgentGraphState,
  latestObservation: PlannerExecutionObservation | undefined,
) => {
  if (latestObservation?.actionType === "tool") {
    ...
    const llmContent = getExecutionLlmContent(execution);
    ...
    return {
      ...
      truncated: llmContent?.truncated ?? false,
      originalCharCount: llmContent?.originalCharCount ?? text.length,
      includedCharCount: Math.min(
        llmContent?.includedCharCount ?? text.length,
        PLANNER_LATEST_EVIDENCE_CONTENT_CHAR_LIMIT,
      ),
      content:
