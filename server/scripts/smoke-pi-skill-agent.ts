...
const parseArgs = (): Cli => {
  ...
  if (!skill || !["docx", "pdf", "pptx", "xlsx"].includes(skill)) {
    return fail("Usage: pnpm smoke:pi-skill-agent -- --skill docx|pdf|pptx|xlsx [--workspace PATH] [--input EXISTING.xlsx]");
  }
  const workspace = path.resolve(
    read("--workspace") ?? path.join(process.cwd(), ".test-artifact", "pi-skill-agent-smoke", skill),
  );
  const inputPath = read("--input");
  return { skill, workspace, inputPath };
};
...
const goalFor = (cli: Cli) => {
  switch (cli.skill) {
    ...
    case "xlsx": {
      if (!cli.inputPath) {
        return fail("XLSX XML-first create/edit bridge is still pending. Pass --input <workspace-relative existing.xlsx> to smoke diagnostics/read instead of faking create support.");
      }
...
const getSkillExecutionEvent = (events: AssistantExecutionNodeEvent[]) =>
  [...events]
    .reverse()
    .find((event) => event.nodeId === "agent-forked-skill-agent" && event.phase === "done");
...
const main = async () => {
  ...
  const threadId = `smoke-pi-skill-agent:${cli.skill}:${runId}`;
  ...
  const firstSkillEvent = getSkillExecutionEvent(events);
  const firstDetails = asRecord(firstSkillEvent?.details);

  print("skillId", cli.skill);
  print("engine", firstDetails?.engine ?? "pi-agent-core");
  print("workspace", cli.workspace);
  print("toolExposure", firstDetails?.allowedHarnessToolIds ?? []);
  print("privateRuntime", firstDetails?.runtimeBindings ?? []);
  print("approval pause/resume", output.status === "waiting_approval" ? "paused" : "not_required");
  ...
  if (output.status === "waiting_approval") {
    ...
    if (!pendingApproval || !pendingToolCall) {
      return fail("waiting_approval output did not preserve exact pendingApproval + pendingToolCall");
    }
  ...
  const skillEvent = getSkillExecutionEvent(events);
  const skillDetails = asRecord(skillEvent?.details);
  const toolCalls = Array.isArray(skillDetails?.toolCalls) ? skillDetails.toolCalls : [];
  const plannerEvents = events.filter((event) => event.nodeId === "agent-next-action-planner");
  ...
  const artifactStats = artifactPaths.map((artifactPath) => {
    ...
    const insideWorkspace =
      absolute === cli.workspace || absolute.startsWith(`${cli.workspace}${path.sep}`);
    if (!insideWorkspace) {
  ...
  print("evidence count", output.evidence.observations.length + output.evidence.toolExecutions.length + output.evidence.retrievals.length);
  ...
  if (output.status !== "completed") {
    return fail(`expected completed output, received ${output.status}: ${output.errorMessage ?? output.blockedReason ?? "unknown"}`);
  }
  if (plannerEvents.length !== 0) {
    return fail("Main Planner emitted execution events after forked Skill ownership; completed Skill was not finalized directly");
  }
