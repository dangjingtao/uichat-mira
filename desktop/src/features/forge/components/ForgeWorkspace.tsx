...
function TaskContext({ task, onDispatch, onReview, onClose }: { task: ForgeTask | null; onDispatch: () => void; onReview: () => void; onClose?: () => void }) {
  if (!task) return <div className="h-full bg-surface-primary"><div className="border-b border-border px-4 py-3 text-caption text-text-tertiary">CURRENT TASK</div><div className="p-4 text-sm text-text-tertiary">No task selected</div></div>;
  const dispatchable = task.readiness === "ready" && (task.runtimeState === "waiting" || task.runtimeState === "fixing");
  const reviewable = task.repositoryState === "REVIEW" && task.runtimeState === "reviewing";
  return <div className="flex h-full flex-col bg-surface-primary"><div className="flex items-center justify-between border-b border-border px-4 py-3"><span className="text-caption text-text-tertiary">CURRENT TASK</span>{onClose ? <IconButton ariaLabel="Close task context" size="sm" onClick={onClose}><X className="h-4 w-4" /></IconButton> : null}</div><div className="stable-scrollbar min-h-0 flex-1 overflow-y-auto p-4"><div className="rounded-ui-panel border border-border bg-surface-secondary p-4"><div className="font-mono text-[11px] text-text-tertiary">{task.id}</div><h2 className="mt-2 text-base font-medium">{task.title}</h2><dl className="mt-4 space-y-2 text-xs"><div className="flex justify-between gap-3"><dt className="text-text-tertiary">Task source</dt><dd className="font-mono text-right text-text-secondary">{task.source}</dd></div><div className="flex justify-between gap-3"><dt className="text-text-tertiary">Dependencies</dt><dd className="text-right text-text-secondary">{task.dependencies.length ? task.dependencies.join(", ") : "None"}</dd></div></dl></div><div className="mt-4"><div className="mb-2 text-caption text-text-tertiary">STATE</div><StatePair task={task} /></div>{task.readiness === "blocked" ? <div className="mt-4 rounded-ui-panel border border-danger-border bg-danger-soft p-3 text-xs leading-5 text-danger-text"><div className="flex items-center gap-2 font-medium"><CircleAlert className="h-4 w-4" />Dispatch unavailable</div><p className="mt-1">{task.dependencies.length ? `Waiting for ${task.dependencies.join(", ")} to reach Integrated.` : "Readiness checks have not passed."}</p></div> : null}<div className="mt-4 space-y-2">{reviewable ? <Button variant="primary" className="w-full" onClick={onReview}>Enter Review</Button> : null}<Button variant={dispatchable ? "primary" : "secondary"} className="w-full" disabled={!dispatchable} onClick={onDispatch}><Play className="h-4 w-4" />{dispatchable ? "Dispatch Builder" : "Dispatch unavailable"}</Button></div></div></div>;
}

function RuntimePanel({ mode, runtimes, events, onClose }: { mode: "summary" | "inspector" | "events"; runtimes: ForgeRuntimeRecord[]; events: ForgeEvent[]; onClose: () => void }) {
  const title = mode === "summary" ? "Runtime Summary" : mode === "inspector" ? "Runtime Inspector" : "Event Log";
  return <Drawer open onClose={onClose} width={420} header={<div className="text-sm font-semibold">{title}</div>}>{mode === "events" ? <div className="space-y-1 font-mono text-xs">{events.length ? events.map((event) => <div key={event.id} className="grid grid-cols-[72px_52px_1fr] gap-2 border-b border-border/70 py-2 text-text-secondary"><span className="text-text-tertiary">{event.timestamp}</span><span className="text-primary">{event.kind}</span><span>{event.message}</span></div>) : <div className="py-8 text-center text-text-tertiary">No events</div>}</div> : <div className="space-y-2">{runtimes.length ? runtimes.map((runtime) => <div key={runtime.id} className="flex items-start justify-between gap-3 border-b border-border/70 py-3"><div className="min-w-0"><div className="truncate text-sm">{runtime.taskId}</div><div className="mt-1 text-xs text-text-tertiary">{runtime.builder} · {runtime.branch ?? "runtime"}</div>{runtime.summary ? <div className="mt-2 text-xs text-text-secondary">{runtime.summary}</div> : null}</div><Badge variant={runtimeVariant(runtime.state)}>{runtimeLabel[runtime.state]}</Badge></div>) : <div className="py-8 text-center text-text-tertiary">No active runtime</div>}</div>}</Drawer>;
}
