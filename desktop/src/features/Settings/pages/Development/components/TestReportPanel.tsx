...
export default function TestReportPanel() {
  ...
  const coverageUrl = `${coverageBaseUrl}/client-coverage/index.html`;
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        ...
        {hasCoverageReport ? (
          ...
          >
            {t("settings.development.clientTests.openFullReport")}
      ...
      {hasCoverageReport ? (
        <div className="overflow-hidden rounded-lg border border-border/70">
          <iframe
            title={t("settings.development.clientTests.reportTitle")}
            src={coverageUrl}
        ...
        <div className="rounded-lg border border-border/70 bg-surface-secondary/60 px-3 py-4 text-sm text-text-secondary">
          {t("settings.development.clientTests.reportNotAvailable")}
        </div>
      )}
    </div>
  );
