    ...
    >
      ...
      <Card padding="sm" className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <Badge variant="neutral">Native Messaging（主连接）</Badge>
        </div>
      ...
      {workspaceTab === "jianxing" ? (
        <div className="grid min-h-0 gap-4 md:grid-cols-2">
          <Card padding="md" className="space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-3"><span className="text-xs text-text-tertiary">{extensionConnected ? "请求将在本机 Native Messaging 通道中发送" : "请在 Chrome 中完成见行扩展授权，连接后才能发送请求"}</span><Button variant="primary" onClick={() => void run()} disabled={busy || !extensionConnected || (isFileUpload && !file)}><Send className="h-4 w-4" />{busy ? "执行中…" : isLook ? "观察页面" : "发送操作"}</Button></div>
          </Card>
        ...
        <Card padding="md" className="space-y-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="text-heading-2 text-text-primary">浏览器主动剪藏</h2>
              <p className="mt-1 text-sm text-text-secondary">剪藏由你在 Chrome 中主动发起，不会作为见行浏览器操作工具被模型调用。</p>
            </div>
            <Badge variant={extensionConnected ? "success" : "warning"}>{extensionConnected ? "扩展已连接" : "等待扩展"}</Badge>
          </div>
          <div className="rounded-ui-control border border-border bg-surface-secondary p-4">
            <div className="text-sm font-medium text-text-primary">当前状态</div>
            <p className="mt-1 text-sm text-text-secondary">{extensionConnected ? "见行扩展已连接，可以在当前网页点击扩展图标进行剪藏。" : "请先安装扩展并完成授权，连接后可在 Chrome 中主动剪藏页面。"}</p>
          </div>
          <div className="flex flex-wrap items-center gap-2 border-t border-border pt-3">
            <Button size="sm" variant="secondary" onClick={() => setExtensionModalOpen(true)}>
              <KeyRound className="h-4 w-4" />浏览器扩展授权
            </Button>
            <span className="text-xs text-text-tertiary">剪藏入口位于 Chrome 工具栏中的见行扩展。</span>
          </div>
        </Card>
      )}
      <Modal
