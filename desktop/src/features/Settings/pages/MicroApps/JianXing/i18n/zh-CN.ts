  ...
  settings: {
    microApps: {
      jianXing: {
        ...
        auth: { title: "浏览器扩展授权", close: "关闭", intro: "生成一次性授权码后，请在 Chrome 的触界授权页粘贴并授权。", generate: "生成授权码", generating: "生成中...", copy: "复制", expiry: "授权码 5 分钟内有效且只能使用一次。授权成功后扩展会自动连接 Mira，无需再手动点击连接。" },
        messages: { nativeReadFailed: "无法读取 Native Messaging 状态", connectExtension: "请先连接触界扩展", rulesLoaded: "URL 剪藏规则已从触界扩展加载", rulesReadFailed: "无法读取 URL 剪藏规则", regionIncludePrompt: "请在 Chrome 中点击正文区域", regionExcludePrompt: "请在 Chrome 中点击要排除的区域", regionIncluded: "已选择当前页面的正文区域", regionExcluded: "已添加当前页面的排除区域", regionFailed: "区域选择失败", urlRequired: "请输入 URL 通配符或正则", invalidRegex: "URL 正则格式无效，请检查括号、反斜杠和量词", rulesSaved: "URL 剪藏规则已保存到触界扩展", rulesSaveFailed: "保存 URL 剪藏规则失败", ruleNotSaved: "当前 URL 规则尚未保存", rulesDeleted: "URL 剪藏规则已删除", rulesDeleteFailed: "删除 URL 剪藏规则失败", connectFailed: "无法连接触界服务", browserFailed: "浏览器操作失败", codeGenerated: "授权码已生成，5 分钟内有效且只能使用一次", codeGenerateFailed: "生成授权码失败", codeCopied: "授权码已复制", extensionDownloaded: "Mira Clipper 已下载到系统下载目录", extensionDownloadFailed: "插件下载失败", nativeInstalled: "Native Messaging 连接组件已安装", nativeNeedsRepair: "Native Messaging 安装后仍需修复", nativeInstallFailed: "Native Messaging 安装失败", nativeUnregistered: "Native Messaging 已解除注册", nativeUnregisterFailed: "Native Messaging 解除注册失败" },
