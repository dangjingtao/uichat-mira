...
const getSampleOverviewFields = (): SampleOverviewDefinition[] => {
  ...
  return [
    {
      label: language === "en-US" ? "Sample" : "样本",
      getValue: (sample) => sample.id,
    },
    {
      label: language === "en-US" ? "Question" : "问题",
  ...
  attempts
    .map((attempt) => {
      ...
      const base = [
        ...
        attempt.status === "success"
          ...
          : language === "en-US"
            ? "Failed"
            : "失败",
        `Latency ${formatSeconds(attempt.latencyMs)}`,
      ...
      const answerLine = attempt.answerText?.trim()
        ? `  - ${language === "en-US" ? "Answer" : "回答"}：${attempt.answerText.trim()}`
        : null;
      const errorLine = attempt.errorMessage
        ? `  - ${language === "en-US" ? "Error" : "错误"}：${attempt.errorMessage}`
        : null;
  ...
  sample.retrievedSources
    .map((source) => {
      ...
      const preview = source.contentPreview
        ? `\n  - ${language === "en-US" ? "Preview" : "预览"}：${source.contentPreview}`
        : "";
      return `- ${meta}${preview}`;
    })
    .join("\n");
...
const buildSampleKeyPoints = (sample: EvaluationSampleResult) => {
  ...
  return [
    ...
    `- ${language === "en-US" ? "Latency" : "耗时"}：${formatSeconds(sample.latencyMs)}`,
    `- Hit：${sample.hit ? (language === "en-US" ? "Yes" : "是") : language === "en-US" ? "No" : "否"}；Source Hit：${
      sample.sourceHit
        ? language === "en-US"
          ? "Yes"
          : "是"
