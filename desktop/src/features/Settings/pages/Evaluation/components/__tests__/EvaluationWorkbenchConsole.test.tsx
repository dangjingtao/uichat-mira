...
import type { EvaluationJobStatus, ParsedDataset } from "../../utils/types";
...
const renderConsole = (overrides: Partial<React.ComponentProps<typeof EvaluationWorkbenchConsole>> = {}) =>
  render(
...
describe("EvaluationWorkbenchConsole", () => {
  ...
  it("switches to result output and shows sample summaries", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "settings.evaluation.workbench.console.log" }));
  ...
  it("shows empty result state when no run exists", () => {
    ...
    expect(screen.getByText("settings.evaluation.workbench.console.emptyResultTitle")).toBeInTheDocument();
    expect(screen.getByText("settings.evaluation.workbench.console.emptyResultDescription")).toBeInTheDocument();
  });
});
