...
// UChatThreadHeader renders the sticky top bar for the shared chat thread UI.
export function UChatThreadHeader({
  title,
  badges,
}: {
  title: string;
      ...
      <div className="flex min-h-10 w-full items-center justify-between gap-3 px-4 py-2 sm:px-6 lg:px-8 xl:px-10">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2 overflow-hidden pl-3">
          {badges.map((item) => {
            ...
            return (
              <Tooltip key={item.key} text={item.name} placement="bottom">
