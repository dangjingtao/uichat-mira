...
(function () {
  ...
  async function waitForSendButton(chatgpt) {
    ...
    while (Date.now() < deadline) {
      ...
      if (button && !button.disabled && !button.hasAttribute('disabled')) return button;
  ...
  async function sendThroughCurrentComposer(chatgpt, message) {
    ...
    while (Date.now() < deadline) {
      ...
      if (latestPrompt === message.trim()) return;
      if (composer instanceof HTMLTextAreaElement && !composer.value.trim()) return;
  ...
  async function sendMessage(sessionRef, message) {
    ...
    const idle = await chatgpt.isIdle(LIBRARY_TIMEOUT_MS);
    ...
    const normalizedReply = typeof reply === 'string' && reply.trim()
      ? reply.trim()
      : String(await chatgpt.getLastResponse?.() || '').trim();
