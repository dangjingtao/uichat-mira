...
见行生产连接固定使用 **Native Messaging**。扩展调用 `chrome.runtime.connectNative()`，Chrome 启动 Mira 安装的 `MiraWebBridgeHost.exe`，Host 再通过本机 IPC 连接 Mira。

WebSocket 实现仍保留在代码中，作为内部开发和兼容通道，但不再出现在 Mira、授权页、Popup 或扩展设置中，用户不能选择或切换到该通道。
