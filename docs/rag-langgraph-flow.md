...
- `details.vectorCount`
- `details.lexicalCount`
- `details.fusedCount`
- `details.retrievalBreakdown`
  - `strategy`
  - `counts.embeddingHits`
  - `counts.lexicalHits`
  - `counts.fusedHits`
  - `candidates[]`
...
- `details.topN`
- `details.counts`
- `details.rerankBreakdown`
  - `applied`
  - `degraded`
  - `finishReason`
  - `counts.inputCandidates`
  - `counts.outputCandidates`
  - `candidates[]`
    - `stage = "input" | "output"`
    - `rank`
    - `chunkId`
...
当前前端展示的 `sources.score` 不是单一来源的绝对分，而是经过“检索召回 -> 可选融合 -> 可选 rerank -> 再归一化”后的最终相对分。

```mermaid
flowchart TD
    A["用户问题"] --> B["rewrite<br/>得到 retrievalQuestion"]
    B --> C["embed<br/>生成 query embedding"]

    C --> D["vector retrieve<br/>sqlite-vec 返回 distance"]
    D --> E{"distanceMetric"}
    E -->|cosine| F["原始向量分 = 1 - distance"]
    E -->|other| G["原始向量分 = -distance"]
    F --> H["向量结果按当前候选集做 min-max 归一化<br/>得到 vector score 0~1"]
    G --> H

    B --> I["lexical retrieve<br/>Orama 检索 documentName + content"]
    I --> J["关键词原始分 = hit.score"]
    J --> K["关键词结果按当前候选集做 min-max 归一化<br/>得到 lexical score 0~1"]

    H --> L{"是否存在问题文本"}
    K --> L
    L -->|否| M["仅保留 vector score"]
    L -->|是| N{"是否同时存在 vector 和 lexical 结果"}
    N -->|否，仅一种命中| O["直接使用该路召回结果"]
    N -->|是| P["RRF 融合<br/>rrfScore = 1 / (60 + rank)"]
    P --> Q["同一 chunk 命中两路时累加 rrfScore"]
    Q --> R["按 rrfScore 排序并截断 topK"]
    R --> S["融合结果再次 min-max 归一化<br/>得到 retrieval score 0~1"]
    M --> T["retrievedChunks.score"]
    O --> T
    S --> T

    T --> U{"rerank 是否启用且调用成功"}
    U -->|否| V["fallback: 按 retrievedChunks.score 排序并截断 topN"]
    U -->|是| W["调用 /v1/rerank 类接口<br/>返回 relevance_score"]
    W --> X["按 scoreThreshold 过滤"]
    X --> Y["按 relevance_score 排序并截断 topN"]
    Y --> Z["rerank 结果再次 min-max 归一化<br/>得到 final score 0~1"]
    V --> AA["sources.score"]
    Z --> AA["sources.score"]

    AA --> AB["前端显示<br/>相关度 {source.score.toFixed(3)}"]
```

### 解释

- 向量检索的原始分来自 `distance`，当前实现中：
  - `cosine` 索引使用 `1 - distance`
  - 其他距离度量使用 `-distance`
- 关键词检索的原始分来自 Orama 返回的 `hit.score`
- 当向量召回和关键词召回同时存在时，最终不是直接混合原始分，而是先按名次做 RRF 融合
- 检索阶段和 rerank 阶段都会再次做一次 `0~1` 归一化，所以前端看到的是“当前候选集内部的相对分”，不是跨请求稳定可比的绝对置信度
- 当 rerank 未启用、未配置完整或调用失败时，最终 `sources.score` 会回退为检索阶段分数
- 当 rerank 成功时，最终 `sources.score` 会被 rerank 的 `relevance_score` 覆盖，再经过当前候选集归一化后展示
