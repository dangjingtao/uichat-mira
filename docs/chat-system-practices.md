...
当前阶段已明确“不做分支切换闭环”，因此消息持久化不能继续累积 assistant-ui 的 sibling branch。

- 非 RAG 对话：
  - `ThreadHistoryAdapter.append()` 提交消息时必须带上 `parentId`。
  - 服务端收到 `parentId` 或 `metadata.assistantUi.branch.parentId` 后，应先裁掉该父节点之后的旧尾巴，再写入新消息。
- RAG 对话：
...
- 历史恢复：
  - 在旧脏数据仍存在时，前端加载层先按“连续 assistant 仅保留最后一条”做线性投影，避免当前 UI 直接把重复回答叠出来。
- 适用边界：
  - 该策略对应本轮“先完成编辑 / 重新生成闭环，不做 branch switch UI”的范围。
  - 如果后续恢复 branch switch，就不能继续使用这种线性裁尾策略，需要把 parent/head 持久化做实。
...
- assistant-ui 消息渲染链路已补齐，附件消息、普通文本消息、来源卡片可在同一线程内工作。
