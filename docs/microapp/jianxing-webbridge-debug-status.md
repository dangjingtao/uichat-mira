...
### Native Messaging 未完成

当前尚未完成以下闭环：

- Chrome 扩展调用 `connectNative()`。
- Chrome 启动安装目录中的 Native Host。
- Native Host 正确继承 Chrome 的 stdin/stdout/stderr。
- Native Host 将 hello 转发到 backend 并返回 `hello_ack`。
- Native Port 断开后 Host 退出且不会遗留旧进程。
