...
- [ ] 包装成对话可选择的洞见知识源
- [ ] 实现原始证据、主题知识页、洞见和观点版本的双层召回
- [ ] 根据问题意图选择事实检索、主题检索、观点检索或混合检索
- [ ] 回答展示 capture、事实单元、洞见和观点版本的完整引用链
- [ ] 支持将高价值的比较、归纳和分析保存为新的知识页或观点版本
- [ ] 支持用户发起知识库健康检查，并展示冲突、过期、孤立页和证据缺口
- [ ] 记录查询命中、引用和回写结果，避免同一问题反复进行高成本全量分析
