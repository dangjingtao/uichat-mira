...
图片快照的数量由当前 URL 剪藏规则的 `imagePolicy.maxCount` 控制，最大为 50。扩展只在用户点击保存时读取图片，并以最多 3 个并发请求上传；每张图片最大 8 MB。任一图片失败不应中断其余图片的保存，也不能把未保存图片的远程 URL 留在原文 Markdown 中。失败项保留在 capture metadata，供用户核对本次快照结果。
