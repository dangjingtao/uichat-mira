...
Planner 只注入问题文本，不看到 Provider、专家实例、conversation、Tab、CDP、DOM、焦点或发送按钮。
...
- 当前 Agent 能力内部默认选择 ChatGPT Expert。Provider 不由 Planner 参数决定；未来切换或路由 Provider 也必须留在能力内部。
- 用户必须先在问策界面点击“创建连接”。只有当前用户的默认 Expert 存在运行时绑定时，Planner 才能看到 `ask_external_expert`。
- Agent 不创建连接、不打开 Chrome，也不恢复旧 Tab；同一进程中已有运行时绑定时，只继续当前外部会话。
- 服务重启或 Tab 失效后不会恢复旧 Tab 或旧线程。连接失效错误会清除运行时绑定，后续 Agent 运行不再暴露该工具，直到用户在问策界面重新创建连接。
- 服务读取当前 Mira thread 的 `contextSummary`，作为有界 `[Mira consultation context]` 段追加到外部咨询。相同摘要在同一运行时连接中只追加一次；摘要变化或新建连接后才再次追加。

完整外部 conversation 标识仍由 ExternalExpert service 持久化和校验，但不会返回给 Planner。
...
External Expert 回复只作为 Tool Result 和 Evidence 返回，不能直接绕过 Planner 成为 Mira 最终回答。Evidence 的 `data` 保留上述结果，`facts` 只记录工具、状态和耗时，不暴露 Provider 或 conversation 标识。
...
- `ask_external_expert` 的 Harness 实现保持全局注册，但 Planner 工具面按当前用户的运行时连接动态过滤；Agent 输入仅为 `question`。
- 共享的 `externalExpertService` 在内部维护默认 Provider、Provider 单例、会话引用、运行时绑定和有界 thread 上下文。Agent 不负责建立连接。
...
- 真实 Agent → ChatGPT WebBridge/CDP → Evidence → Planner 冒烟已通过。
