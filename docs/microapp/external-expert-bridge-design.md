...
- 当前 Expert 没有运行时连接时，service 内部建立新的 Provider 页面和空白会话；同一进程中已有运行时绑定时，继续当前外部会话。
- 服务重启或 Tab 失效后不会恢复旧 Tab 或旧线程；下一次咨询内部建立新的连接。
...
- `ask_external_expert` 已注册到 Harness，Agent 输入仅为 `question`；共享的 `externalExpertService` 在内部维护默认 Provider、Provider 单例、会话引用、运行时绑定和有界 thread 上下文。
