...
- 已增加 forked Skill parent-routing 回归测试，覆盖 completed finalization、needs_input freeze、recoverable failure 与 terminal failure 的控制权边界。
...
2. 必须在安装完成后真实执行 server typecheck 与新增 focused tests；未拿到命令成功结果前不能声称通过。
