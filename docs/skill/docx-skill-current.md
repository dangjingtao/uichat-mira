...
2. 在 Harness 已注册能力中确保以下能力进入唯一 `toolExposure`：
   - `read_locate`
   - `read_open`
   - `read_extract`
   - `office_document`
3. 把 DOCX Skill 的路由、硬规则和完成标准注入 `office_document` 的 Planner-visible metadata；
4. Planner 仍只从现有 `toolExposure` 选择 `use_tool`；
...
Create / Review 后应通过现有 Read 路径重新打开产物，确认：
