...
| Remote Pairing Compatibility | `IN_PROGRESS` | T005：配对 claim 的 requestedScopes 改为向前兼容；route 接受有界字符串，service 只保留 Host 已知 scope，授权边界不放松 | [remote_pairing_T005](tasks/remote_pairing_T005-forward-compatible-scope-negotiation.md) | MOB-044 真机配对失败后收口 Desktop 跨版本硬耦合；不改 Relay / Agent 合同 |
