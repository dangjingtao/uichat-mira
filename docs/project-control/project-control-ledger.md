...
| MCP Agent Job Release | `IN_PROGRESS` | 外部 MCP 市场应用接入 Agent 的资格、候选暴露与黑盒调用闭环；按 T001 → T002 → T003 顺序审查 | [mcp_agent_T001](tasks/mcp_agent_T001-external-mcp-agent-eligibility.md), [mcp_agent_T002](tasks/mcp_agent_T002-external-mcp-agent-exposure-and-selection.md), [mcp_agent_T003](tasks/mcp_agent_T003-external-mcp-agent-invocation-blackbox.md) | T001 已通过本地主审查；T002/T003 继续等待前置任务通过 |
...
| [mcp_agent_T002](tasks/mcp_agent_T002-external-mcp-agent-exposure-and-selection.md) | `BLOCKED` | MCP Agent / Exposure and Selection | 任务卡原始 `task_state: BLOCKED_BY_T001`；T001 通过后进入审查 |
