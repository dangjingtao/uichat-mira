...
| [agent_node_T017](tasks/agent_node_T017-toolcall-loop-regression-matrix.md) | `TASK_CARD_STATE_MISSING` | AgentGraph | 任务卡缺 `task_state`，旧 workboard 标 DONE；需要先修任务卡元数据 |
...
| [agent_node_T017](tasks/agent_node_T017-toolcall-loop-regression-matrix.md) | `TASK_CARD_STATE_MISSING` | missing `task_state` | 文件 `status: done`，旧 workboard 标 DONE；按规则不能由总台账替任务卡判定状态，先修任务卡元数据 |
