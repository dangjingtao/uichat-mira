...
| `agent_node_T016` | Local Tool Routing and Schema Guard Under Weak Task Model | `T016` 是 `Agent V1.5 P0` 修复任务，只补 workspace local intent、防 schema invalid 直接打死前台、以及 generate 空回答 fallback 的最小防线。`2026-07-05` 真实前台 smoke 已完成第二轮复测：`P0-8` 现在会稳定进入本地 `read_locate`，不再误走 `web_search`，也不再落到递归上限；最终回答已回到 `README.md / AGENTS.md / package.json / electron-builder.yml` 等本地证据，不再被 `release/*.exe` 安装包路径带偏。当前剩余现象只剩“第二次完全相同检索”没有单独形成第二条 completed evidence，回答会对此做保守说明；该现象已记录为非阻断解释，因此状态更新为 `READY_FOR_REVIEW` | `READY_FOR_REVIEW` | [agent_node_T016-local-tool-routing-and-schema-guard.md](D:/workspace/rag-demo/docs/project-control/tasks/agent_node_T016-local-tool-routing-and-schema-guard.md) |
